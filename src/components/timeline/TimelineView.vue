...
const currentTitle = computed(() => {
    return filterType ? filterType.replace('-', ' ') : 'Timeline';
});
...
<template>
  ...
  >
    ...
    <div class="fixed top-0 left-0 w-full p-6 z-40 bg-gradient-to-b from-black via-black/90 to-transparent md:bg-none md:w-auto pointer-events-none">
    ...
    <div class="hidden md:flex flex-row items-center h-full px-[50vw] w-max relative pt-0">
      <div class="central-line absolute top-1/2 left-0 w-full h-1.5 bg-white -translate-y-1/2 z-0 shadow-[0_0_15px_white]"></div>
      ...
      >
        <div class="observer-target absolute top-1/2 left-1/2 w-1 h-1 -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-0" :data-id="movie.id"></div>
    ...
    <div class="md:hidden relative w-full min-h-screen py-[50vh] flex flex-col gap-24">
      <div class="central-line absolute top-0 left-1/2 h-full w-1.5 bg-white -translate-x-1/2 z-0 shadow-[0_0_15px_white]"></div>
